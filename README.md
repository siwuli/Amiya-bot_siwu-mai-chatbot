...
> 插件 id：`siwu-mai-chatbot`　当前版本：`1.12.0`

## 特性

- **召唤聊天**：`兔兔…` / `@兔兔…` / 自定义触发词 → 由 LLM 生成符合人设的短回复
- **多消息连发（v1.11）**：一次触发可以连发 1~3 条短消息（条数由模型自己判断，不写死），每条间隔 1~1.5 秒，像真人分段说话
- **对话续聊（v1.11 / v1.12）**：回复成功后进入 120 秒「对话窗口」，窗口内别人继续说话，兔兔会直接续聊（不再评分），最多续几轮可配；v1.12 起**主动发言（评分接话）成功后同样开启窗口**，主动挑起的话题也能接得住
...
## 多消息连发与对话续聊（v1.11 / v1.12）

**多消息连发**：触发回复时，模型被要求可以自主决定连发几条短消息——只回一句就输出一行，觉得需要就连发 2~3 条（每条单独一行，代码上限 3 条）。程序逐行后处理并截断，第一条立即发送，后续每条间隔 1~1.5 秒，模拟真人分段说话。

**对话续聊**：**任何一次成功回复**（被召唤、评分接话、主动发言）后都会进入「对话窗口」（默认 120 秒，`mai_followup_window_seconds` 可配，0 = 关闭）。窗口内别人继续说话时，兔兔**直接续聊、不再评分**——别人回了你，你就能继续跟下去。窗口最多续 `mai_followup_max_turns` 轮（默认 4），防止没人理也无限接话。v1.12 起主动发言（评分接话）成功后同样开启窗口，主动挑起的话题也能接得住。

```yaml
mai_followup_window_seconds: 120   # 对话窗口时长（秒），0 = 关闭续聊
mai_followup_max_turns: 4          # 窗口内最多续聊几轮
```
...
| `mai_user_prompt_template` | 生成时的 user 模板；可多行输出（一行一条，最多 3 行） | `{history}` `{speaker_name}` `{focus_text}` `{bot_name}` |
...
| 版本 | 更新内容 |
|---|---|
| `1.12.0` | 对话窗口扩展：主动发言（评分接话 / 旧主动逻辑）成功后同样开启对话续聊窗口，主动挑起的话题也能接得住；消息密度统计统一由观察路径记录（judge / legacy 共用一份，主动发言的活跃度估算更准确） |
| `1.11.0` | 多消息连发：一次触发可连发 1~3 条短消息（条数由模型自主判断，不写死），逐行后处理截断，每条间隔 1~1.5s 发送；对话续聊：被召唤回复后进入可配窗口（默认 120s），窗口内别人继续说话直接续聊不再评分，最多续 N 轮防无限接话（`mai_followup_window_seconds` / `mai_followup_max_turns`） |
