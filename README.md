...
**对话续聊**：被召唤回复成功后进入「对话窗口」（默认 120 秒，`mai_followup_window_seconds` 可配，0 = 关闭）。窗口内别人继续说话时，兔兔**直接续聊、不再评分**——别人回了你，你就能继续跟下去。窗口最多续 `mai_followup_max_turns` 轮（默认 4），防止没人理也无限接话。
